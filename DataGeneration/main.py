...
MAX_EPISODES=24 #number of episodes of the training
...
epsilon_decay = 1./100000 #this is ok for a simple task like inverted pendulum, but maybe this would be set to zero for more
                     ...
                     #to zero.
PRINT_EVERY = MAX_EPISODES #Print info about average reward every PRINT_EVERY
...
for episode in range(MAX_EPISODES):
    ...
    ep_q_value = 0.
    step=0
    # noise = np.random.choice([-1,0,1],p=[0.3,0.4,0.3])
    ...
    for step in range(MAX_STEPS):
        ...
        actor.load_state_dict(torch.load('models/model.pkl',map_location='cuda:0'))
        #model.eval()
        a = actor.get_action(s) * 0# - noise
        env.render()

        # collecting data
        # if s[1] < 0 and s[0] < 0:
        #     realtheta = -np.arccos(s[0]) + 2*np.pi
        #     # thetada.append(realtheta/np.pi*180)
        #     thetada.append(realtheta)
        # if s[1] < 0 and s[0] > 0:
        #     realtheta = np.arcsin(s[1]) + 2*np.pi
        #     # thetada.append(realtheta/np.pi*180)
        #     thetada.append(realtheta)
        # if s[1] > 0 and s[0] > 0:
        ...
        costhe.append(s[0])
        sinthe.append(s[1])
        theta_dot.append(s[2])
        action_m.append(a)
        print(a)
        # print('theta is: ', realtheta/np.pi*180)
        print('\n')
        #actor.train()
...
history = np.zeros((4, global_step))
history[0,:] = costhe
history[1,:] = sinthe
history[2,:] = theta_dot
history[3,:] = action_m
